...
(() => {
  ...
  if (hasGSAP && !reduced) {
    ...
    // line/word mask reveals — pure CSS transition toggled by a class.
    // (GSAP yPercent desyncs from real height after the late Fontshare reflow,
    //  so we avoid height-relative units entirely.) Per-word stagger via delay.
    $$("[data-lines]").forEach((el) => {
      const target = el.querySelector("p") || el;
      const words = target.textContent.trim().split(/\s+/);
      target.innerHTML = words
        .map((wd) => `<span class="ln"><span>${wd}</span></span>`)
        .join(" ");
      el.classList.add("is-split");
      [...el.querySelectorAll(".ln > span")].forEach((sp, i) => {
        sp.style.transitionDelay = (i * 0.045).toFixed(3) + "s";
      });
    });
    const revealLines = (el) => el.classList.add("is-in");
    if ("IntersectionObserver" in window) {
      const lineIO = new IntersectionObserver((entries, obs) => {
        entries.forEach((e) => { if (e.isIntersecting) { revealLines(e.target); obs.unobserve(e.target); } });
      }, { rootMargin: "0px 0px -10% 0px", threshold: 0.12 });
      $$("[data-lines]").forEach((el) => lineIO.observe(el));
      // safety: reveal anything still hidden shortly after load
      setTimeout(() => $$("[data-lines]:not(.is-in)").forEach(revealLines), 2600);
    } else {
      $$("[data-lines]").forEach(revealLines);
    }

    // generic reveals — IO-driven opacity/translate, with fallback
    const revIO = "IntersectionObserver" in window ? new IntersectionObserver((entries, obs) => {
      entries.forEach((e) => {
        if (!e.isIntersecting) return;
        gsap.to(e.target, { opacity: 1, y: 0, duration: 0.9, ease: "power3.out" });
        obs.unobserve(e.target);
      });
    }, { rootMargin: "0px 0px -8% 0px", threshold: 0.1 }) : null;

    $$("[data-reveal]").forEach((el) => {
      if (revIO) revIO.observe(el); else gsap.set(el, { opacity: 1, y: 0 });
      setTimeout(() => { if (gsap.getProperty(el, "opacity") < 0.9) gsap.to(el, { opacity: 1, y: 0, duration: 0.5 }); }, 2600);
    });
